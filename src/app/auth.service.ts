...
import { data as adjectives } from "./adjectives.json";
import { data as animals } from "./animals.json";
 ...
 console.log(displayName);
    ...
    const data = {
     ...
     const collectiondata = {
      ...
      location: location,
      radius: radius,
     users : [{user:credential.user.uid, displayName: displayName}]
...
this.uid = credential.user.uid;
    ...
    var linklocation = this.router.url;

	linklocation = linklocation.split('/').pop();
    console.log(linklocation);
    this.afs.collection("restaurants").doc(linklocation).update({
    	users: firestore.FieldValue.arrayUnion({user:credential.user.uid, displayName: displayName})
    });
        return this.userRef.set(data, { merge: true });
  }

...
export class MoviesComponent implements OnInit {
    ...
    showMatch(data) {
        ...
        Swal.fire({
            title: "IT'S A MATCH!",
            html: "<img src ='" + data.image + " height='100', width = '100'> <br><b>" + data.rating + "/10<br></b><br>" + "Plot: " + data.synopsis + "<br>",
            footer: "<a href='http://www.netflix.com/' target='blank'>Powered by <img src = '/assets/Netflix.png' height = '40'></a>"
    ...
    showMap(data) {
        var urlstring = "https://www.netflix.com/title/";
        urlstring = urlstring + data.netflixid;
    ...
    copylink() {
        ...
        this.clipboard.copy("noyoupick.io/movies/" + this.roomID)
    ...
    infoButton() {
        ...
        Swal.fire({
            title: 'NoYouPick!',
            text: 'This is the lobby. Wait for the host to start, then begin swiping on your favorite movies!',
            icon: 'info',
            imageWidth: 400,
            imageHeight: 200,
        });
    ...
    changevoteyes(max) {
        ...
        this.choices.push(1);
        if (this.index < (max - 1)) {
            if ((this.index + 1) % 5 == 0) { //if this is your 5th, 10th, 15th.. vote, send data to server
    ...
    changevoteno(max) {
        ...
        //checks every 5 indexes. has to be +1 b/c arrays start at 0
        this.choices.push(0);
        if (this.index < (max - 1)) { //if this is your 5th, 10th, 15th.. vote, send data to server
            if ((this.index + 1) % 5 == 0) {
                this.updateScores(false);
            }
            this.index = this.index + 1;
    ...
    copylinkalert() {
        Swal.fire({
            title: 'Welcome!',
            html: 'Share this link! <br> noyoupick.io/movies/' + this.roomID,
            icon: 'success',
    ...
    ngOnInit() {
        ...
        this.metaTagService.updateTag({
            name: 'description',
            content: 'NoYouPick - Tinder, but for food! Lobby in progress.'
        });
    }
